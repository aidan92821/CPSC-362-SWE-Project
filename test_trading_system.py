...
def backtest(df, initial_balance=100000, stock='SOXL'):
    ...
    log_data (list): A list of lists containing the log for CSV export.
    """
    ...
    log_data = [["DATE", "buy/sell", "STA", "LTA", "balance", "amount of stock"]]
...
def run_sma_and_backtest(file_path, stock, start_date=None, end_date=None):
    ...
    df = calculate_sma(df)
    log, log_data = backtest(df, stock=stock)
